...
void heat_diffusion_step(float *T, float *T_new, int N, int boundary_row, float alpha1, float alpha2) {
	#pragma omp parallel for
    for (int i = 1; i < N - 1; i++) {
		#pragma omp simd
        for (int j = 1; j < N - 1; j++) {
            float alpha = (i < boundary_row) ? alpha1 : alpha2;
            T_new[i * N + j] = (1 - alpha) * T[i * N + j] + 
...
int main(int argc, char *argv[]) {
    ...
    if (N < 10 || boundary_row < 1 || boundary_row >= N - 1 || 
        alpha1 < 0.0f || alpha1 > 1.0f || alpha2 < 0.0f || alpha2 > 1.0f || 
        iterations < 1) {
